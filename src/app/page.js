...
export default function Home() {
  ...
  const download = (item) => {
    ...
    a.href = url;
    a.download = "tinypix-" + item.original.name;
    a.click();
  ...
  const downloadAllZip = async () => {
    ...
    files.forEach((item) => {
      if (item.compressed)
        zip.file("tinypix-" + item.original.name, item.compressed);
    });
  ...
  const totalBefore = compressedFiles.reduce(
    (s, f) => s + f.original.size,
    0
  );
  const totalAfter = compressedFiles.reduce(
    (s, f) => s + f.compressed.size,
    0
  );
  const totalSavedPct =
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center px-4 pb-20 z-10">
      ...
      <header className="w-full max-w-3xl flex items-center justify-between py-6 relative z-10">
        ...
        <span className="text-xs text-white/50 glass px-3 py-1.5 rounded-full">
      ...
      <section className="text-center mt-10 mb-10 relative z-10 fade-up">
        ...
        <p className="text-white/60 mt-4 max-w-md mx-auto">
          Instant compression right in your browser — your images never leave
          your device. Free, fast, unlimited.
        </p>
      </section>

      {/* Drop zone — visual signature */}
      <div className="w-full max-w-2xl relative z-10 fade-up">
        <div className="glow-border">
          <div
            ...
            onDrop={handleDrop}
            className={`glow-border-inner p-14 text-center cursor-pointer transition-all duration-300
              ${isDragging ? "scale-[1.02] bg-[#1b1440]" : "hover:bg-[#181234]"}`}
          >
        ...
        <div className="w-full max-w-2xl relative z-10">
          ...
          <div className="glass rounded-2xl p-5 mt-4 fade-up">
            ...
            <p className="text-xs text-white/40 mt-2">
              Lower quality = smaller size · 70% is the sweet spot
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3 mt-5 justify-center fade-up">
            ...
            {compressedFiles.length > 1 && (
              <button
                ...
                className="bg-gradient-to-r from-[#a78bfa] to-[#7c3aed] hover:brightness-110 px-7 py-3.5 rounded-2xl font-bold transition shadow-lg shadow-[#7c3aed]/30"
              >
                📦 Download all (ZIP)
              </button>
            )}
            <button
              onClick={reset}
              title="Clear all"
              className="glass hover:bg-white/10 px-5 py-3.5 rounded-2xl transition"
            >
              🗑️
            </button>
          </div>
          ...
          <div className="mt-6 space-y-3">
            {files.map((item, i) => (
              <div
                key={i}
                className="glass rounded-2xl p-3 flex items-center gap-3 fade-up hover:bg-white/[0.06] transition"
              >
                <img
                  src={item.preview}
                  alt=""
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/50 text-xs mt-0.5">
                    ...
                    {item.compressed && (
                      <>
                        {" → "}
                        <span className="text-[#ff7a59] font-bold">
      ...
      <footer className="mt-auto pt-16 text-center text-white/30 text-xs relative z-10">
        <p>
          TinyPix ⚡ Made with ❤️ in Morocco 🇲🇦 — your images never get stored
          anywhere
        </p>
      </footer>
    </main>
